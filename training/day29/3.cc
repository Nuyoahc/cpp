// https://www.nowcoder.com/practice/4af96fa010c44638a7e112abf65f7237?tpId=196&tqId=39285&ru=/exam/oj
// 给定一个长度为 n 的数组a，求它的最长严格上升子序列的长度。
// 所谓子序列，指一个数组删掉一些数（也可以不删）之后，形成的新数组。
// 例如 [1,5,3,7,3] 数组，其子序列有：[1,3,3]、[7] 等。但 [1,6]、[1,3,5] 则不是它的子序列。
// 我们定义一个序列是 严格上升 的，当且仅当该序列不存在两个下标 i 和 j，使得 i < j 且 a[i] >= a[j]。
// 输入：
// [1,4,7,5,6]
// 返回值：
// 4
// 说明：
// 最长上升子序列为 [1,4,5,6] ，长度为4。 
#include <iostream>
#include <vector>
using namespace std;

class Solution {
    int f[100001] = {0}; // f[i] 表示长度为 i 的最长上升子序列的末尾元素的最小值
    int pos = 0; // pos 表示当前最长上升子序列的长度
public:
    /**
     * 代码中的类名、方法名、参数名已经指定，请勿修改，直接返回方法规定的值即可
     *
     * 该数组最长严格上升子序列的长度
     * @param a int整型vector 给定的数组
     * @return int整型
     */
    int LIS(vector<int>& a) {
        // write code here
        int n = a.size();
        for(int i = 0; i < n; i++)
        {
            // 查找放在哪个位置
            if(pos == 0 || a[i] > f[pos])
            {
                f[++pos] = a[i];
            }
            else
            {
                // 二分查找，找到第一个大于等于 a[i] 的位置
                int left = 1, right = pos;
                while(left < right)
                {
                    int mid = (left + right) / 2;
                    if(f[mid] < a[i])
                        left = mid + 1;
                    else
                        right = mid;
                }
                // 更新该位置的值
                f[left] = a[i];
            }
        }
        return pos; // 返回最长上升子序列的长度
    }
};

int main()
{
    vector<int> a{1,4,7,5,6};
    Solution solution;
    cout << solution.LIS(a) << endl; // 输出 4

    return 0;
}